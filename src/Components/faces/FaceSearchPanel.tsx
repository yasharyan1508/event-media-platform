...
export function FaceSearchPanel({ faceIndexes }: { faceIndexes: any[] }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`/api/search?type=faces&faceIndexId=${selectedFace}`)
      .then(res => res.json())
      .then(data => {
        if (data.success) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 max-w-4xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="text-xl font-bold text-gray-900">Global Face Search</h2>
      </div>
      <p className="text-gray-500 text-sm mb-6">
        Select an enrolled face to find all their photos across every event on the platform.
      </p>
        ...
        {faceIndexes.map((face) => (
          <button
            ...
            className={`relative flex-shrink-0 w-16 h-16 rounded-full overflow-hidden border-2 transition-all duration-300 ${
              selectedFace === face.id ? "border-indigo-600 scale-110 shadow-lg shadow-indigo-200" : "border-transparent hover:scale-105"
            }`}
          >
            {face.user?.avatarUrl ? (
      ...
      {selectedFace && !loading && results.length > 0 && (
        <div className="mt-6 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
          {results.map((m) => (
            <div key={m.id} className="relative aspect-square rounded-xl overflow-hidden shadow-sm group ring-1 ring-black/5 hover:ring-indigo-500/50 transition">
      ...
      {selectedFace && !loading && results.length === 0 && (
        <div className="py-12 text-center bg-gray-50 rounded-xl border border-gray-100 mt-6">
          <p className="text-gray-500">No photos found across any events.</p>
        </div>
      )}
    </div>
  );
